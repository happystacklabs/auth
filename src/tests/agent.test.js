...
describe('agent', () => {
  ...
  describe('delete', () => {
    it('make a delete request and return the body', (done) => {
      const spy = jest.fn();
      agent.requests.delete('/').then(spy);
      moxios.wait(() => {
        const request = moxios.requests.mostRecent();
        request.respondWith({
          status: 201,
  ...
  describe('uploadAvatar', () => {
    it('make a post request to user/avatar', () => {
      agent.requests.post = jest.fn();
      agent.requests.post.mockImplementation(() => ({ response: 'foo' }));
      expect(agent.Auth.uploadAvatar('foobar')).toEqual({ response: 'foo' });
    });
  });

  describe('removeAvatar', () => {
    it('make a delete request to user/avatar', () => {
      agent.requests.delete = jest.fn();
      agent.requests.delete.mockImplementation(() => ({ response: 'foo' }));
      expect(agent.Auth.removeAvatar()).toEqual({ response: 'foo' });
    });
  });
});
